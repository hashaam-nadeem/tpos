...
import 'package:transact/utils/cashieraddress.dart';
import 'package:transact/utils/cashiercheckoutmethod.dart';
...
class _BuyerCartState extends State<SellerMycart> {
  int counter = 1;
  var listItems = 6;
...
ProgressDialog pr;
  ...
  deleteCart(int id) async
  {
    ...
    print(header);
    var t="${API.deleteCashierCartLine}?id=$id";
    print(t);
    var response = await http.get(
      t,
      headers: header,
  ...
  getCartline() async
  {
    ...
    var response = await http.get(
      "${API.getCashierCart}?pin=${User.userData.rememberPin}",
      headers: header,
    );
    var Json = json.decode(response.body);
    print(json.decode(response.body));
