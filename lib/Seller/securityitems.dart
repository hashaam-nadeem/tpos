...
import 'package:transact/Model/pinmodulesmodel.dart';
...
class SecurityItems extends StatefulWidget {
  @override
  _PINState createState() => _PINState();
}

class _PINState extends State<SecurityItems> {
  ...
  var style1 =
      ...
      PinsmoduleModel pinsmoduleModel = PinsmoduleModel();
      ProgressDialog pr;


                assignModuleSwitch(int moduleId) async {
    pr.show();
    var header = {
   ...
   getPinsModules()async
    ...
    var response = await http.get(
      "${API.ViewPinsModules}?Pin=${User.userData.pin}",
      headers: header,
    );
    var Json = json.decode(response.body);
    print(json.decode(response.body));
    if(response.statusCode==200)
    {
      if(Json['Data']['WithError']==false)
      {
        pr.dismiss();
        setState(() {
          pinsmoduleModel=PinsmoduleModel.fromJson(Json['Data']);
        });
      ...
      else
      {
        pr.dismiss();
        setState(() {
          pinsmoduleModel=PinsmoduleModel();
        });
         Fluttertoast.showToast(
              msg: "${Json['Data']['ShortMessage']}",
              textColor: Colors.white,
    ...
    return Container(
      ...
      height: MediaQuery.of(context).size.height,
        ...
        children: <Widget>[
          ...
          child: Text(
            "---- Select Items to be Secured with Pin -----",
           ...
           Expanded(
             child: ListView.builder(itemBuilder: (BuildContext context,int index)
               ...
               itemCount: pinsmoduleModel.result!=null?pinsmoduleModel.result.length:0,
               ),
           ),
        ],
      ),
    ...
    return Container(
      ...
      children: <Widget>[
       Row(
        ...
        children: <Widget>[
          Text(
            "${pinsmoduleModel.result[index].moduleName}",
            style: style1.copyWith(
                color: Colors.black, fontSize: 15),
          ),
          Switch(
            value: pinsmoduleModel.result[index].isAssign,
            onChanged: (value)
            {
              toggleSwitch(pinsmoduleModel.result[index].id);
            }
          )
        ],
      ),
       // _list("${pinsmoduleModel.result[index].moduleName}"),
        // _list("Marketplace", 3),
        // _list("Cashier", 4),
